<template>
  <div>
    <div class="tw-h-full lg:tw-h-[60vh] tw-bg-no-repeat tw-bg-cover tw-bg-center"
      ...
      <div class="tw-flex tw-flex-col tw-items-center tw-justify-center tw-px-4 2xl:tw-px-0">
        <form
          class="tw-bg-white tw-rounded-lg tw-h-full tw-my-20 tw-gap-4 tw-p-3 md:tw-p-6 lg:tw-p-12 tw-flex tw-flex-col">
          <div class="tw-inline-flex tw-gap-4 md:tw-gap-8 tw-text-xl tw-font-light">
            <span class="tw-inline-flex tw-items-center">
              <input type="radio" class="tw-w-5 tw-h-5 checked:tw-bg-red-600" value="true"
                v-model="reservationForm.aller_simple" checked id="aller_simple" name="voyage">
              <label for="aller_simple" class="tw-ml-2">Aller Simple</label>
            </span>
            <span class="tw-inline-flex tw-items-center">
              <input type="radio" class="tw-w-5 tw-h-5 checked:tw-bg-red-600 tw-bg-gray-500" value="false"
                v-model="reservationForm.aller_simple" id="aller_retour" name="voyage">
          ...
          <div class="tw-flex tw-flex-wrap tw-items-center tw-justify-between tw-gap-4 md:tw-gap-6 tw-w-full">
            <div class="tw-flex tw-gap-3 tw-items-center tw-bg-white">
              <v-autocomplete v-model="reservationForm.airport_depart" :items="departs" :loading="loadingDeparts"
                :search-input.sync="searchDeparts" clearable hide-details hide-selected item-text="name"
                item-value="_id" label="Choisissez l'adresse de départ..." outlined height="70">
              ...
              <v-autocomplete class="" v-model="reservationForm.airport_destination" :items="destinations"
                :loading="loadingDestinations" :search-input.sync="searchDestinations" clearable hide-details
                hide-selected item-text="name" item-value="_id" label="Choisissez l'adresse d'arrivée..." outlined
                height="70">
                <template v-slot:no-data>
                  <v-list-item>
                    <v-list-item-title>
            ...
            <v-menu ref="depart_menu" v-model="depart_menu" :close-on-content-click="false" :return-value.sync="date"
              transition="scale-transition" offset-y min-width="auto">
              <template v-slot:activator="{ on, attrs }">
                <v-text-field v-model="reservationForm.depart_date" label="Date de départ" readonly height="70" outlined
                  v-bind="attrs" v-on="on"></v-text-field>
              </template>
              <v-date-picker v-model="reservationForm.depart_date" no-title scrollable>
                <v-spacer></v-spacer>
                <v-btn text color="primary" @click="depart_menu = false">
                  Cancel
                </v-btn>
                <v-btn text color="primary" @click="$refs.depart_menu.save(date)">
                  OK
                </v-btn>
              </v-date-picker>
            </v-menu>
            ...
            <v-menu ref="menu" v-model="menu" :close-on-content-click="false" :return-value.sync="date"
              transition="scale-transition" offset-y min-width="auto">
              <template v-slot:activator="{ on, attrs }">
                <v-text-field v-model="reservationForm.comeback_date" label="Date de retour" readonly outlined
                  height="70" v-bind="attrs" v-on="on"></v-text-field>
              </template>
              <v-date-picker v-model="reservationForm.comeback_date" no-title scrollable>
                <v-spacer></v-spacer>
                <v-btn text color="primary" @click="menu = false">
                  Cancel
                </v-btn>
                <v-btn text color="primary" @click="$refs.menu.save(reservationForm.comeback_date)">
                  OK
                </v-btn>
              </v-date-picker>
            </v-menu>
            ...
            <div class="tw-w-full">
              <v-menu :key="text" solo :close-on-content-click="false" offset-y>
                <template v-slot:activator="{ attrs, on }">
                  <v-text-field class="tw-text-xl tw-font-bold" readonly v-bind="attrs" v-on="on"
                    label="Nombre de passagers" outlined height="70" v-model="totalPassagers" dense></v-text-field>
                </template>
                  ...
                  <v-list-item>
                    <v-list-item-title>
                      <div class="tw-flex tw-flex-col tw-gap-4 tw-w-full tw-p-4 tw-bg-white">
                        <div class="tw-py-4 tw-flex tw-justify-between tw-gap-12">
                          ...
                          <div class="tw-text-xl tw-font-bold tw-gray-800 tw-flex tw-items-center tw-gap-4">
                            <div
                              ...
                              <span @click="reservationForm.passengers.adultes--" class="hover:tw-cursor-pointer">
                        ...
                        <div class="tw-py-4 tw-flex tw-justify-between tw-gap-12">
                          ...
                          <div class="tw-text-xl tw-font-bold tw-gray-800 tw-flex tw-items-center tw-gap-4">
                            <div v-if="reservationForm.passengers.enfants > 0">
                              <span @click="reservationForm.passengers.enfants--" class="hover:tw-cursor-pointer">
                                <v-icon color="red">mdi-minus-circle-outline</v-icon>
                              </span>
                            </div>
                            <div v-else>
                              <span class="hover:tw-cursor-no-drop">
                                <v-icon color="grey">mdi-minus-circle-outline</v-icon>
                              </span>
                            </div>
                        ...
                        <v-divider></v-divider>
                        <div class="tw-py-4 tw-flex tw-justify-between tw-gap-12">
                          <span class="tw-text-xl tw-font-bold tw-gray-800">Bébés (< 2 ans)</span> <div
                              class="tw-text-xl tw-font-bold tw-gray-800 tw-flex tw-items-center tw-gap-4">
                              <div v-if="reservationForm.passengers.bebes > 0">
                                <span @click="reservationForm.passengers.bebes--" class="hover:tw-cursor-pointer">
                                  <v-icon color="red">mdi-minus-circle-outline</v-icon>
                                </span>
                              </div>
                              <div v-else>
      ...
      <div class="tw-flex">
        <template>
          <v-row justify="center">
            <v-dialog v-model="userInfoDialog" persistent max-width="600px">
              <template v-slot:activator="{ on, attrs }">
                <v-btn class="" color="error darken-1" dark v-bind="attrs" v-on="on">
              ...
              <v-card>
                ...
                <v-card-text>
                  <v-container>
                    <v-row>
                      <v-col cols="12" sm="6">
                        <v-text-field label="Nom*" required outlined v-model="reservationForm.lastname"></v-text-field>
                      </v-col>
                      <v-col cols="12" sm="6">
                        <v-text-field label="Prénoms" required outlined v-model="reservationForm.firstname">
                        </v-text-field>
                      </v-col>
                      ...
                      <v-row cols="12" class="tw-mx-2">
                        <v-col sm="6">
                          <v-autocomplete v-model="reservationForm.phone_number.code" :items="countries"
                            :loading="isLoading" :search-input.sync="search" clearable item-text="dial_code"
                            item-value="id" outlined label="Indicatif de votre numéro">
                            <template v-slot:item="{ item }">
                              <v-list-item-avatar color="indigo" class="text-h10 tw-p-4 font-weight-light white--text">
                                {{ item.dial_code}}
                              </v-list-item-avatar>
                              <v-list-item-content>
                        ...
                        <v-text-field v-model="reservationForm.phone_number.number" justify="center" required
                          type="number" label="Numéro de télephone" outlined></v-text-field>
                      </v-row>
                    </v-row>
                  </v-container>
                ...
                <v-card-actions>
                  <v-spacer></v-spacer>
                  <v-btn color="error darken-1" @click="userInfoDialog = false" text>
                    Fermer
                  </v-btn>
                  <v-btn class="" color="error darken-1" :loading="btnLoading"
      ...
      <template>
        <div class="text-center">
          <v-dialog v-model="disclaimerDialog" width="500">
            ...
            <v-card>
              ...
              <v-card-actions>
                <v-spacer></v-spacer>
                <v-btn color="primary" text @click="disclaimerDialog = false">
                  Annuler
                </v-btn>
                <v-btn color="primary" text @click="reservation()">
                  J'accepte
                </v-btn>
              </v-card-actions>
            </v-card>
  ...
  <div class="tw-mx-auto tw-px-4 tw-my-20">
    <div class="tw-flex tw-flex-wrap tw-justify-evenly tw-gap-8">
      <div class="">
        <svg class="tw-h-12 tw-fill-current tw-text-gray-500" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 266 100">
      ...
      <div class="">
        <svg class="tw-h-10 tw-fill-current tw-text-gray-500" xmlns="http://www.w3.org/2000/svg"
      ...
      <div class="">
        <svg class="tw-mt-2 tw-h-8 tw-fill-current tw-text-gray-500" viewBox="0 0 398 120"
      ...
      <div class="">
        <svg class="tw-mt-1 tw-h-5 tw-fill-current tw-text-gray-500" xmlns="http://www.w3.org/2000/svg"
      ...
      <div class="">
        <svg class="tw-h-8 tw-fill-current tw-text-gray-500" viewBox="0 0 2270 546" xmlns="http://www.w3.org/2000/svg">
          <g fill-rule="evenodd">
            <path
  ...
  export default {
    ...
    computed: {
      totalPassagers() {
        ...
        return this.reservationForm.passengers.adultes + this.reservationForm.passengers.enfants + this.reservationForm
          .passengers.bebes
      },
    ...
    methods: {
      async reservation() {
        ...
        await axios.post(
          'http://cf5c-2c0f-f0f8-2be-f800-6c43-f02f-e42d-5944.ngrok.io/reservation-vol/request-flight-reservation',
          this.reservationForm).then((response) => {
