    ...
    lazy-validation
          ...
          <div class="tw-flex tw-flex-col tw-gap-6 tw-mt-6 tw-w-full">
            <div class="tw-inline-flex tw-w-full tw-gap-6 tw-items-center">
              <div class=" tw-text-lg tw-rounded-lg tw-w-full tw-flex tw-flex-col tw-p-3">
                ...
                <v-text-field v-model="form.lieu_prise_en_charge"
      :rules="lieuPriseEnChargeRules" required outlined placeholder="Lieu de prise en charge">
              ...
              <div class=" tw-text-lg tw-rounded-lg tw-w-full tw-flex tw-flex-col tw-p-3">
                ...
                <v-text-field v-model="form.lieu_de_restitution"
      :rules="LieuRestitutionRules" required outlined placeholder="Lieu de restitution">
            ...
            <div class="tw-flex tw-items-center tw-w-full tw-gap-4">
              <div class=" tw-text-lg tw-rounded-lg tw-w-full tw-flex tw-flex-col tw-p-3">
                ...
                <v-menu ref="debut_menu" v-model="debut_menu" :close-on-content-click="false" :return-value.sync="date"
                  ...
                  <template v-slot:activator="{ on, attrs }">
                    <v-text-field v-model="form.date_debut" placeholder="Date de début: 05/12/2022" readonly outlined
                      height="70" v-bind="attrs" v-on="on"></v-text-field>
                  </template>
                  <v-date-picker v-model="form.date_debut" no-title scrollable>
                    <v-spacer></v-spacer>
                    <v-btn text color="primary" @click="debut_menu = false">
                      Cancel
                    </v-btn>
                    <v-btn text color="primary" @click="$refs.debut_menu.save(form.date_debut)">
                      OK
                    </v-btn>
                  </v-date-picker>
                </v-menu>
              ...
              <div class=" tw-text-lg tw-rounded-lg tw-w-full tw-flex tw-flex-col tw-p-3">
                ...
                <v-menu ref="fin_menu" v-model="fin_menu" :close-on-content-click="false" :return-value.sync="date"
                  transition="scale-transition" offset-y min-width="auto">
                  <template v-slot:activator="{ on, attrs }">
                    <v-text-field v-model="form.date_fin" placeholder="Date de fin : 05/12/2022" readonly outlined
                      height="70" v-bind="attrs" v-on="on"></v-text-field>
                  </template>
                  <v-date-picker v-model="form.date_fin" no-title scrollable>
                    <v-spacer></v-spacer>
                    <v-btn text color="primary" @click="fin_menu = false">
                      Cancel
                    </v-btn>
                    <v-btn text color="primary" @click="$refs.fin_menu.save(form.date_fin)">
                      OK
                    </v-btn>
                  </v-date-picker>
                </v-menu>
            ...
            <v-col sm="6">
                <v-textarea  rows="3" v-model="form.description_voiture" label="Faites une breve description de la voiture que vous voulez" name="input-7-1"
                  outlined></v-textarea>
              </v-col>
          </div>
